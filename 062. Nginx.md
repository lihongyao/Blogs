...
> 提示：安装完成后，在浏览器访问服务器 IP 地址，看到 `Welcome to nginx!` 页面即表示成功
...
└─ http块
    ├─ http全局指令（如gzip、日志格式）
    ├─ server块（虚拟主机1）
    │   ├─ server全局指令（如listen、server_name）
    │   └─ location块（URL路径处理）
    └─ server块（虚拟主机2）
        └─ ...
```
...
2. **Events块**
   ...
   - **说明**：`events`块必须配置，直接影响并发性能

3. **HTTP块**

   - **作用**：定义HTTP服务的全局配置，可嵌套多个`server`块。
   ...
   - **说明**：`http`块是配置HTTP服务的核心，支持日志格式、缓存、反向代理等功能
...
4. **Server块**
   ...
   - **说明**：一个`http`块可包含多个`server`块，用于多站点托管
...
5. **Location块**
   ...
   - **常见用途**：
     ...
     - 静态文件处理：`root`或`alias`指定路径。
     - 反向代理：`proxy_pass`转发请求到后端。
     - 访问控制：`allow`/`deny`限制IP

   - **优先级**：精确匹配（`=`） > 前缀匹配（`^~`） > 正则匹配（`~`或`~*`） > 通用前缀匹配
...
## 配置文件示例
   ...
   ```nginx
   # - [全局配置块] - 
   # 配置运行Nginx的用户和组，建议使用专用用户(如nginx)
   user nginx;
   
   # 工作进程数，auto表示自动设置为CPU核心数（推荐生产环境使用），默认为1
   worker_processes auto;
   
   
   # 错误日志配置
   # 路径：/var/log/nginx/error.log
   # 级别：notice（记录重要通知和错误，比warn更详细但比info更简洁）
   ...
   # - [事件驱动模块配置] -
   events {
       # 单个工作进程的最大并发连接数（需根据服务器内存调整）
       # 理论最大并发 = worker_processes × worker_connections
       worker_connections 1024;
       
       # 可选优化参数（根据系统类型启用）：
       # use epoll;        # Linux高效事件模型
       # multi_accept on;  # 允许进程同时接受多个新连接
   }
   
   
   # - [HTTP服务核心配置] -
   http {
       # 包含MIME类型定义文件（扩展名到Content-Type的映射）
       include       /etc/nginx/mime.types;
   
       # 默认响应Content-Type（当无法识别文件类型时使用）
       default_type  application/octet-stream;
       ...
       sendfile_max_chunk 100k; # 限制每次sendfile调用的传输量（避免worker进程阻塞）
       tcp_nopush on;           # 仅在sendfile开启时有效，优化网络包发送效率
   
   
       # 连接保持配置
       keepalive_timeout 65;   # 客户端长连接超时时间（秒）
       keepalive_requests 100; # 单个长连接允许的最大请求数（建议添加）
   
       # 性能优化建议（根据需求取消注释）：
       # tcp_nodelay on;           # 禁用Nagle算法，减少小数据包延迟
       # client_max_body_size 10m; # 客户端请求体最大尺寸（上传文件时需要调整）
   ...
   server {
       ...
       # 请求控制
       client_max_body_size 70m; # 允许最大70MB请求体（适用于文件上传场景）
       add_header Accept-Ranges bytes; # 启用字节范围请求（支持断点续传功能）
   
       # SSL会话配置
       ssl_session_timeout 5m;  # SSL会话缓存超时时间（降低握手开销）
       ssl_session_cache shared:SSL:10m;  # 共享SSL会话缓存（提升性能）
   
       # 安全协议配置（生产环境建议仅保留TLSv1.2+）
       ssl_protocols TLSv1.2 TLSv1.3;  # 禁用不安全的TLSv1.0/1.1
       ssl_ciphers 'ECDHE-ECDSA-AES256-GCM-SHA384:ECDHE-RSA-AES256-GCM-SHA384';  # 现代加密套件
       ssl_prefer_server_ciphers on;  # 优先使用服务端加密套件
   
       # 证书配置（需替换实际路径）
       ssl_certificate /etc/nginx/certs/xxx.cn.pem;  # 证书链文件（包含中间证书）
       ssl_certificate_key /etc/nginx/certs/xxx.cn.key;  # 私钥文件
