...
- `nvm current`：要查看当前使用版本。

- `nvm ls` / `nvm list`：显示已安装的所有 版本。

- `nvm alias <name> <version>`：为特定版本创建别名

  `nvm alias default 14.17.0`：将 14.17.0 版本设置为默认版本。
...
由于npm是国外的服务器，在国内使用很多时候会不稳定，出现下载 “假死” 或速度过慢的现象（如果出现这样的情况我们可以 `ctr + C` 结束进程）。为了解决这个问题阿里集团的一个团队为npm创建了一个国内的服务器镜像——阿里镜像（cnpm），它以10分钟为单位保持与国外npm服务器的同步频率。也就是说我们直接可以通过阿里镜像去下载或更新我们的程序包，而不必去忍受缓慢的程序包下载速度了。
...
2. 登陆
   ...
   > 注意：如果你将镜像文件设置成阿里镜像，将会登陆失败，在终端输入如下指令切换至官方源即可：
