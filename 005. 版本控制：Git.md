...
我们采用的是一种 **“基于环境的分支发布模型”**。

![](./imgs/git_branch_strategy.png)
...
我们不使用复杂的 GitFlow，而是用更轻量、更可控的方式。我们将从几个问题开始介绍

**1️⃣ 为什么功能必须从 prod 拉出？**
...
👉 合并到 dev 分支，由 dev 统一发测试环境。

dev 的存在，就是为了做“集成测试”。

**3️⃣ 为什么不能 dev 直接合到 stage？**
...
如果直接：dev → stage，那体验服就会变成“混合版本”，无法控制发布内容。

**4️⃣ 体验服阶段怎么修 Bug？**

当功能合到 stage 后，如果出现问题，不要直接改 stage。正确方式是：
...
体验服验证通过后：feature → main，然后打 tag 发布。

main 永远只接受已验证通过的功能或 hotfix 修复，这样生产环境始终干净、稳定、可回溯。

**6️⃣ 线上紧急修复怎么做？**

如果线上出现严重 Bug，必须从 main 拉 hotfix 分支。修复完成后：
...
这时就不能直接用 feature 合 stage。

需要引入：release/x-v1.0.0

流程变为：feature → release → stage → main

release 分支只做版本冻结和 bug 修复，不接受新功能。
...
速查用；需要参数说明时：`git help <命令>` 或 `git <命令> -h`。

## 配置

- 初始化仓库：`git init`
- 初始化并指定初始分支名：`git init -b main`
- 配置用户名：`git config --global user.name "…"`
- 配置邮箱：`git config --global user.email "…"`
- 查看配置：`git config --list`
- 默认分支名为 main：`git config --global init.defaultBranch main`
...
- 查看远程分支：`git branch -r`
- 查看全部分支：`git branch -a`
- 查看分支与上游：`git branch -vv`
- 切换分支：`git switch <分支>`
- 切换分支（旧写法）：`git checkout <分支>`
- 新建并切换分支：`git switch -c <新分支>`
- 新建并切换分支（旧写法）：`git checkout -b <新分支>`
- 从远程分支创建本地分支：`git switch -c <新分支> origin/<远程分支>`
- 重命名当前分支：`git branch -m <新名>`
- 删除已合并的本地分支：`git branch -d <分支>`
- 强制删除本地分支：`git branch -D <分支>`
- 推送并建立上游跟踪：`git push -u origin <分支>`
...
- 按块暂存（交互）：`git add -p`
- 提交：`git commit -m "…"`
- 直接提交已跟踪文件（等同先 add）：`git commit -a -m "…"`
- 修改上次提交：`git commit --amend`
- 跳过 Git 钩子提交：`git commit -n -m "…"`
- 从暂存区撤回文件：`git restore --staged <路径>`
- 丢弃工作区未暂存修改：`git restore <路径>`
...
- 删除远程名：`git remote remove <名>`
- 获取远程更新（不合并）：`git fetch`
- 获取全部远程并清理已删分支引用：`git fetch --all --prune`
- 拉取并合并当前分支：`git pull`
- 拉取指定远程分支：`git pull origin <分支>`
- 推送到上游：`git push`
- 推送到指定远程分支：`git push origin <分支>`
- 强制推送（相对更安全）：`git push --force-with-lease`

## 合并 / 变基 / 挑提交

- 合并分支：`git merge <分支>`
- 合并并保留合并提交节点：`git merge --no-ff <分支>`
- 中止合并：`git merge --abort`
- 变基到某分支之上：`git rebase <分支>`
- 交互式整理提交：`git rebase -i HEAD~n`
- 变基冲突解决后继续：`git rebase --continue`
- 中止变基：`git rebase --abort`
- 挑选指定提交到当前分支：`git cherry-pick <commit>`（常用操作见下文）

## 回退

- 软回退（保留暂存与工作区）：`git reset --soft HEAD~1`
- 混合回退（默认）：`git reset --mixed HEAD~1`
- 硬回退（丢弃本地修改）：`git reset --hard HEAD~1`
- 回到指定提交：`git reset --hard <commit>`
- 用新提交撤销某次提交：`git revert <commit>`（常用操作见下文）
- 引用日志：`git reflog`
- 简洁线图日志：`git log --oneline --graph --decorate`

## 暂存

- 暂存当前修改：`git stash`
- 暂存并写说明：`git stash push -m "…"`
- 暂存并包含未跟踪文件：`git stash -u`
- 列出 stash：`git stash list`
- 应用最近一条（不删）：`git stash apply`
- 应用最近一条并删除：`git stash pop`
- 删除指定条：`git stash drop stash@{n}`
- 清空全部 stash：`git stash clear`

## 标签

- 列出标签：`git tag`
- 创建轻量标签：`git tag <名>`
- 创建附注标签：`git tag -a <名> -m "…"`
- 推送单个标签：`git push origin <tag>`
...
- 工作区与暂存区差异：`git diff`
- 暂存区与上次提交差异：`git diff --cached`
- 查看当前提交：`git show`
- 查看某次提交中的文件内容：`git show <commit>:<路径>`
- 查看文件提交历史：`git log --follow -- <路径>`
- 逐行查看归属：`git blame <路径>`

## 克隆

- 克隆仓库：`git clone <url>`
- 克隆并递归子模块：`git clone --recurse-submodules <url>`
- 拉取并初始化子模块：`git submodule update --init --recursive`
- 预览将删除的未跟踪文件：`git clean -n`
- 删除未跟踪文件与目录：`git clean -fd`

## cherry-pick / revert（常用扩展）

日常一般就这些。

### cherry-pick

- 挑一条提交到当前分支：`git cherry-pick <commit>`
- 按顺序挑几条：`git cherry-pick <c1> <c2>`
- 有冲突：解决后 `git add`，再 `git cherry-pick --continue`，想整段放弃用 `git cherry-pick --abort`

### revert

- 用新提交「反做」某次改动（适合已推送、不想改历史的场景）：`git revert <commit>`
- 要撤销的是**合并提交**时：`git revert -m 1 <merge_commit>`
- 有冲突：解决后 `git add`，再 `git revert --continue`，放弃用 `git revert --abort`

# 扩展

## GitHub Desktop

[**GitHub Desktop ↪**](https://desktop.github.com/)  是免费的开放源代码应用程序，可帮助处理托管在 GitHub 或其他 Git 托管服务上的文件。

使用指南：https://docs.github.com/zh/desktop/overview
