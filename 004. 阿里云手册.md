...
参考 [新手上云指南 ↪](https://help.aliyun.com/product/47310.html) 完成 <u>注册账号</u> 和 <u>个人实名认证</u> 或 <u>企业认证</u>。
...
- 系统盘：**ESSD 云盘 - 40GiB**（可降至 **20GiB** 以节省成本），其他默认
...
- 前往 [万网 ↪](https://wanwang.aliyun.com/) 查询 → 购买域名；

- 前往 [ICP 备案 ↪](https://beian.aliyun.com/)，点击右下角 **我要备案** 然后根据提示进行即可；
  - 管局审核时间约为 10天
  - 建议域名购买3天后再去备案（工信部同步数据）
  - 提示：备案需购买至少3个月的ECS服务器才让你备（万恶的捆绑销售呀）

- 前往 [云解析 DNS ↪](https://wanwang.aliyun.com/domain/dns)，进入控制台 → 域名解析 → 选中要解析的域名点击操作 / **解析设置** → 添加记录
...
1. 开通阿里云短信服务
   ...
   - 登录 [阿里云平台短信服务 ↪](https://www.aliyun.com/product/sms)

   - 完成企业实名认证（个人账号无法申请短信签名）
...
1. 前往 [数字证书管理服务 ↪](https://www.aliyun.com/product/cas) — 登录控制台 — 证书管理 — SSL证书管理 — 个人测试证书（原免费证书）
...
3. 申请证书
   ...
   - 填写证书绑定域名：your_domain.com （可省略 www）
   - 域名验证方式：自动DNS验证
   - 联系人：x x x
   - 所在地：x x x
   - 密钥算法：RSA_2048
...
4. 当证书申请通过后，会在证书列表中显示 **已签发** 状态

   下载证书：在「操作」栏选择「更多」，切换至「下载」项，根据服务器类型下载，我选择的是 **Nginx**。

5. 登录云服务器，将下载的证书放置在 *`/etc/nginx/ssl`* 目录下
...
1. 访问 [阿里云容器镜像服务控制台 ↪](https://cr.console.aliyun.com/)，选择「个人版」或「企业版」（个人开发者可免费使用个人版）
2. 创建命名空间
   - 在「命名空间」页面点击「创建」
   - 填写唯一名称（如公司名/项目名），需全小写且不超过64字符
3. 创建镜像仓库
   - 选择目「镜像仓库」 → 点击「创建镜像仓库」
   - 填写仓库名称（建议与应用名一致，如`nginx`）
   - 选择仓库类型：私有/公开（公开仓库可被匿名拉取）
   - 仓库信息：
     - 代码源：Codeup（阿里云云效）
     - 构建设置：
       - ☑️ 代码变更自动构建镜像
