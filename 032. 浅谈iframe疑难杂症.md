...
X-Frame-Options: ALLOW-FROM https://example.com  # 仅允许特定域名（部分浏览器支持）
```

> 📖 **使用 `Content-Security-Policy (CSP)` 头（更灵活）**

CSP 提供了更细粒度的控制，可以指定允许嵌套的域名：

```ini
Content-Security-Policy: frame-ancestors 'none';         # 完全禁止
Content-Security-Policy: frame-ancestors 'self';         # 仅允许同域名
Content-Security-Policy: frame-ancestors https://example.com;  # 仅允许特定域名
```
...
这样即使某些旧浏览器不支持 CSP，也能通过 `X-Frame-Options` 防护。
